...
```
    - We will never hit the base case "return 1 if n == 1" since we're subtracting increments of 2 from 10. 10 -> 8 -> 6 -> 4 -> 2 -> 0. This causes a stack overflow.
...
Write a recursive function that prints all the numbers (and just numbers).
    ...
    element = first element of array
    if element is array
        print_numbers(element)
    else
        puts element
    print_numbers(rest of array)
end
